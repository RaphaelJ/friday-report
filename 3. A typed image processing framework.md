...
To understand how you can benefit from a typed image processing framework, one 
...
Masked images are images which are not defined for every coordinates. They are
used when you want to ignore some areas of an image. For example, you will
use a masked image if you don't want algorithms to be applied on the background
part of the image.
...
In *friday*, masked images are a distinct image type which share some type
